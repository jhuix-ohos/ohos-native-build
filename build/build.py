...
modifiedEnv['OHOS_SDK'] = sdkDir.replace('\\', '/') if win else sdkDir
...
def stage(name, commands, location = libsLoc):
    ...
    if win:
        ...
        if not isLib or len(archs) == 0:
            ...
            for arch in archs.split(','):
                runEnv = modifiedEnv.copy()
                buildEnv = ohosenv.setEnv(arch, runEnv['OHOS_SDK'])            
                for key in buildEnv:
                    runEnv[key] = buildEnv[key]
                result = subprocess.run("command.bat", shell=True, env=runEnv).returncode == 0
                if not result:
                    break
...
def runStages():
    ...
    for stage in stages:
        ...
        if not run(commands, stage['directory'] == libsDir):
            print(prefix + ': FAILED')
            finish(1)
        writeCacheKey(stage)
...
def loadLib(name):
    ...
    if not 'location' in result:
        stage(result['name'], result['commands'])
    else:
        stage(result['name'], result['commands'], result['location'])
    return True        
